defmodule EventManagerWeb.HomeLive do
  ...
  def mount(_params, _session, socket) do
    ...
    total_hours = Enum.reduce(my_certificates, 0, fn cert, acc -> 
      duration = if Map.has_key?(cert, :event) && cert.event, do: Map.get(cert.event, :duration_minutes, 240) || 240, else: 240
      hours = round(duration / 60)
      acc + hours
    end)
    
    next_event = my_events 
